...
### Radar & satellite maps (camera entities)

Optional `camera.*` entities with maps generated by IMGW API Proxy. 800×800 px image with OSM basemap, IMGW data overlay and location marker. Refreshes every 5 minutes (radar) or 15 minutes (satellite).
...
#### Satellite imagery (refreshes every 15 minutes)
...
| Option | Description |
|---|---|
| Single product | Any radar or satellite map |
| Radar: All (3 maps) | CMAX + SRI + PAC |
| Satellite: All (4 images) | Natural color + Cloud cover + Water vapor + Cloud type |
| Everything (7 maps) | Radar + Satellite |

Radar images include color scales with values (dBZ, mm/h, mm). Cloud cover and cloud type images include color legends. Every image includes measurement date and time.
...
IMGW API Proxy (forecast, radar, station coordinates):
- `https://imgw-api-proxy.evtlab.pl/forecast` (weather forecast)
- `https://imgw-api-proxy.evtlab.pl/radar?lat=...&lon=...&product=...` (radar & satellite maps)
