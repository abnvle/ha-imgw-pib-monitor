...
- **Up to 90+ entities** - temperature, wind, humidity, pressure, water level, flow, trends, alarms, warnings, phenomenon binary sensors, radar maps and more
- **Weather entity** - weather forecast (daily and hourly) from IMGW-PIB as a weather entity
- **Radar & satellite maps** - optional camera entities with precipitation, reflectivity, 1h accumulation and satellite imagery
...
- **Dynamic station coordinates** - synoptic station coordinates fetched from API (fallback to local data)
- **Fault tolerance** - radar map and forecast failures don't block the rest of the integration
...
### Radar, satellite & OZE maps (camera entities)

Optional `camera.*` entities with maps generated by IMGW API Proxy. 800×800 px image with OSM basemap, IMGW data overlay and location marker.

#### Radar maps (refreshes every 5 minutes)

| Map | Description | Product |
|---|---|---|
| Reflectivity (CMAX) | Maximum radar reflectivity — shows cloud and precipitation echoes | `cmax` |
| Rainfall (SRI) | Surface rainfall intensity in mm/h | `sri` |
| 1h accumulation (PAC) | Accumulated precipitation over the last hour in mm | `pac` |

#### Satellite imagery (refreshes every 5 minutes)

| Map | Description | Product |
|---|---|---|
| Natural color | Satellite image in natural colors | `natural_color` |
| Cloud cover (IR) | Infrared cloud map — works 24/7, regardless of time of day | `infrared` |
| Water vapor | Water vapor channel 6.2µm — air mass movement | `water_vapor` |
| Cloud type | NWC SAF cloud classification — low, medium, high, Cb | `cloud_type` |

#### OZE — Renewable Energy forecasts (refreshes every full hour)

Renewable energy generation forecast maps based on the ECMWF IFS 9km model. Data identical to meteo.imgw.pl, updated every full UTC hour. Animations show a 24-hour forecast ahead.

| Map | Description | Product |
|---|---|---|
| PV generation | Photovoltaic generation forecast (% of nominal power) | `oze_pv` |
| Wind generation | Wind generation forecast (% of nominal power) | `oze_wind` |
| PV animation (GIF) | Animated GIF with 24h PV forecast | `oze_pv_anim` |
| Wind animation (GIF) | Animated GIF with 24h wind forecast | `oze_wind_anim` |

<table>
<tr>
<td width="50%">

**PV generation forecast (24h animation)**

![PV animation](docs/pv_animation.gif)

</td>
<td width="50%">

**Wind generation forecast (24h animation)**

![Wind animation](docs/wind_animation.gif)
...
Multi-select available in configuration — choose any combination of products:

| Category | Products |
|---|---|
| Radar | CMAX, SRI, PAC |
| Satellite | Natural color, Cloud cover (IR), Water vapor, Cloud type |
| OZE | PV generation, Wind generation, PV animation, Wind animation |

Radar images include color scales with values (dBZ, mm/h, mm). Cloud cover and cloud type images include color legends. OZE maps include generation scale (0-100%). Every image includes date and time in Polish timezone.
...
8. Select radar/satellite map (disabled, single product or all)
...
IMGW API Proxy (forecast, radar, satellite, OZE, station coordinates):
- `https://imgw-api-proxy.evtlab.pl/forecast` (weather forecast)
- `https://imgw-api-proxy.evtlab.pl/radar?lat=...&lon=...&product=...` (radar, satellite & OZE maps)
- `https://imgw-api-proxy.evtlab.pl/radar?...&product=oze_pv&hour=0` (OZE forecast — current hour)
- `https://imgw-api-proxy.evtlab.pl/radar?...&product=oze_pv&animate=24` (OZE animation 24h)
- `https://imgw-api-proxy.evtlab.pl/stations/synop` (synoptic station coordinates)

OZE data (source: ECMWF IFS 9km model via IMGW):
- `https://tilesources-c.imgw.pl/vector/oze/epsg3857/latest/` (OZE forecast images)
